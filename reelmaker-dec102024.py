...
# Define margins as a fraction of the frame dimensions
TOP_MARGIN_PERCENT = 0.2     # Ignore top 20%
BOTTOM_MARGIN_PERCENT = 0.2  # Ignore bottom 20%
LEFT_MARGIN_PERCENT = 0.05   # Ignore left 5%
RIGHT_MARGIN_PERCENT = 0.05  # Ignore right 5%

# Timing and sensitivity parameters
INITIAL_PERIOD_SECONDS = 5.0      # For the first 5 seconds, be extra sensitive
FRAMES_PER_SEGMENT_SHORT = 15     # During initial period, update camera every 15 frames (~0.5s if fps ~30)
FRAMES_PER_SEGMENT_LONG = 30      # After initial period, update camera every 30 frames (~1s if fps ~30)

# Max speed of camera movement in pixels per frame (for smooth panning)
MAX_SPEED = 3

# Minimum movement threshold before camera moves
MIN_MOVEMENT_THRESHOLD = 100  # If target shift < 10px, camera doesn't move

# Debug flag - set to True to produce debug output video
DEBUG = False
...
def compute_crop_positions(face_positions, motion_positions, frame_dims, fps, output_aspect_ratio=9/16.0):
    ...
    for face_bbox, motion_bbox in zip(face_positions, motion_positions):
        ...
        # Check if candidate center is inside safe zone
    ...
    # Replace None with previous valid or center of frame if no previous
    ...
    segment_boundaries = []
    index = 0
    # Initial period with short segments
    while index < min(initial_period_frames, total_frames):
        ...
        index = end_idx
    # After initial period with long segments
    ...
    if not segment_centers:
        # No centers? Just stay centered in the frame
    ...
    # Precompute for each frame: the interpolated target center
    for frame_idx in range(total_frames):
        ...
        if seg_idx is None:
            # Just fallback to last center
            target_x, target_y = segment_centers[-1]
        else:
            # Interpolate between segment_centers[seg_idx] and segment_centers[seg_idx+1] if exists
        ...
        if dist > MIN_MOVEMENT_THRESHOLD:
            # If we exceed threshold, we move
            if dist > MAX_SPEED:
        ...
        try:
            ...
            if DEBUG:
                ...
                def debug_process_frame(get_frame, t):
                    ...
                    # Draw red checkered pattern in margins
                    draw_checkered_pattern(frame_bgr, (0,0), (w_frame, top_limit), (0,0,255))       # Top margin
                    draw_checkered_pattern(frame_bgr, (0,bottom_limit), (w_frame, h_frame), (0,0,255)) # Bottom margin
                    draw_checkered_pattern(frame_bgr, (0,0), (left_limit, h_frame), (0,0,255))         # Left margin
                    draw_checkered_pattern(frame_bgr, (right_limit,0), (w_frame, h_frame), (0,0,255))  # Right margin

                    # Draw motion in green
                    ...
                    # Draw the final cropping rectangle in yellow
                    if frame_idx < len(crop_rects):
                        x1_crop, y1_crop, output_width, output_height = crop_rects[frame_idx]
