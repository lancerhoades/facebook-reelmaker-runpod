...
def main():
    ...
    for video_file in video_files:
        ...
        try:
            ...
            # Process video with computed crops on the fly
            clip = mpy.VideoFileClip(input_video)
            ...
            print(f"Skipping {input_video} due to memory error: {e}")
            continue
        except cv2.error as e:
            if "Insufficient memory" in str(e) or "bad allocation" in str(e):
                print(f"Skipping {input_video} due to OpenCV memory error: {e}")
                continue
            else:
